-- Cap / product lines may use unit_type "bundle" on sales_order_items while
-- prepare_order reserves caps as "loose" in cap_stock_balances (physical pieces).
-- Previous WHERE required exact unit_type match → dispatch found zero rows.
-- Factory: (factory_id = x OR factory_id IS NULL) fails when x IS NULL (SQL NULL = NULL is unknown).
...
COMMENT ON FUNCTION public.process_partial_dispatch IS 'Dispatch via dispatch_records; cap/product deduction matches reserved rows when SOI unit_type bundle vs balance loose, and NULL factory on order matches any factory row';

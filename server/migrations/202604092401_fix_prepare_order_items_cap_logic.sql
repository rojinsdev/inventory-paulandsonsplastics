-- Fix: Update prepare_order_items_atomic to use specific cap_id for tub+cap combinations
-- When preparing orders with tub+cap combinations, the system should reserve stock
-- from the specific combination (product_id + cap_id) rather than any available stock.
-- This ensures accurate stock reservations and prevents over-allocation.
...
BEGIN
    ...
    LOOP
        ...
        INTO v_item.product_id, v_cap_id, v_item.unit_type, v_include_inner, v_inner_id, v_factory_id
        ...
        IF v_item.product_id IS NOT NULL THEN
            ...
            IF v_item.unit_type = 'loose' THEN
                v_source_state := 'semi_finished';
            ELSIF v_item.unit_type = 'packet' THEN
            ...
            IF v_available_stock < v_item.quantity THEN
                RAISE EXCEPTION 'Insufficient physical stock for product %. Required: %, Available: % in state %', 
                    v_item.product_id, v_item.quantity, v_available_stock, v_source_state;
            END IF;
            ...
            LOOP
                ...
                INSERT INTO public.stock_balances (
                    ...
                    v_item.product_id, v_factory_id, v_to_reserve, 'reserved', v_item.unit_type, 
        ...
        ELSIF v_cap_id IS NOT NULL THEN
            ...
            LOOP
                ...
                INSERT INTO public.cap_stock_balances (
                    ...
                    COALESCE(v_item.unit_type, 'loose'), NOW()

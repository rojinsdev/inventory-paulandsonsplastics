...
class SalesOrderService {
    async createOrder(data) {
        ...
        for (const item of data.items) {
            ...
            if (itemError) {
                ...
                await supabase_1.supabase.from('sales_orders').delete().eq('id', order.id);
    ...
    async reserveStock(productId, quantityBundles) {
        // Get finished stock
        ...
        // Get reserved stock
        const { data: reservedStock } = await supabase_1.supabase
            .from('stock_balances')
            .select('quantity')
            .eq('product_id', productId)
            .eq('state', 'reserved')
    ...
    async unreserveStock(productId, quantityBundles) {
    ...
    async deliverStock(productId, quantityBundles) {
